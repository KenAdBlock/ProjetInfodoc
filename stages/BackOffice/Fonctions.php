...
// ========================
	...
	//
    //Fonction s�curisant les strings dans les Formulaires
...
{
    // S�paration du nom_prenom et du groupe
   ...
   // S�paration du nom et des pr�noms
   ...
   // Recomposition et mise en minuscules de tous les pr�noms
 
   $Prenoms = $TabStr [1];
   for ($i = 2; $i < count ($TabStr); ++$i) $Prenoms .= ' '.$TabStr [$i];
   $Prenoms = strtolower ($Prenoms);
   
   // Compactage de tous les pr�noms
   
   $PrenomsCompactes = str_replace ('-', '', $Prenoms);
   
   // Mise en majuscule de l'initiale de chaque pr�nom
    ...
    for (; $Ligne = fgets($handle); )
    {
	    ...
	    else
	    {
	        ...
	        print ($Nom.' '.$Prenoms.' '.$NumGroupe.';'.$LoginAuto.
	               ' Ancien nom =  '.$Obj->Nom.' Ancien pr�nom =  '.$Obj->Prenom.'<br />');
        }
        /*
